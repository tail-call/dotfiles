...
function window.build()
    ...
    r.layout:pack(r.buf)
    r.layout:pack(r.ssl)
    --r.layout:pack(r.tabi) -- Why do I need it anyway?
...
window.methods = {
    ...
    update_progress = function (w)
        ...
        if not w.view:loading() or p == 1 then
            ...
            local function bar (divisions, progress)
                local completed = math.ceil(divisions * progress)
                return string.rep("#", completed) .. 
                       string.rep("-", divisions - completed)
            end
            loaded.text = string.format("[%s] %d%%", bar(5, p), p * 100)
            loaded:show()
        end
    end,
    ...
    update_scroll = function (w)
        ...
        -- One "page" is kinda 600 pixels high
        text = string.format("%d/%d", math.floor(y/720)+1, math.ceil(max/720))
