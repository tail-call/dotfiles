...
add_binds("normal", {
    ...
    buf("^g[Ff]$", "View the source code of the current document.",
        --function (w, a, o) w:toggle_source(not o.bang and true or nil) end),
        function (w) w:toggle_source() end),
...
--      function (w) w:close_win() end),
    ...
    buf("^,d$", "DuckDuckGo search",
        function (w) w:enter_cmd(":open quack ") end),

    buf("^,h$", "Hackage search",
        function (w) w:enter_cmd(":open quack !hackage ") end),

    buf("^,n$", "Netflix search",
        function (w) w:enter_cmd(":open netflix ") end),

    key({}, "D", "Close current tab (or `[count]` tabs) and go left",
        function (w, m) for i=1,m.count do w:close_tab(); w:prev_tab(); end end, {count=1}),
})
...
readline_bindings = {
    ...
    key({"Mod1"}, "Insert", "Edit text in external editor.",
        function (w)
		local editor = "xterm -e vim"
		local time = os.time()
		local file = "/tmp/" .. time
...
add_cmds({
    ...
    cmd("echo", "Echo a message.",
        function (w, a) w:notify(a) end),
    ...
    cmd("tabf[irst]", "Switch to first tab.",
        function (w) w:goto_tab(1) end),
    ...
    cmd("lua", "Evaluate Lua snippet.", function (w, a)
        if a then
            local ret = assert(
                ...
                loadstring("return function(w) "..a.." end"))()(w)
    ...
    cmd({"video", "vid"}, "Add a video to ponych.at queue (:vid url)",
        function (w, a) w.view:eval_js("Meteor.call('removeVideo'); Meteor.call('addVideo', {url: '"..a.."'})") end),

    cmd({"macrosay", "ms"}, "Says a sentence to ponych.at replacing $(expr) forms with the result of evaluating expr.",
        function (w, a)
            ...
            w:notify(expand(a))
            w.view:eval_js("Meteor.call('post', {content: '"..expand(a).."'})")
        end),
